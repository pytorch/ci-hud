...
export default class BuildHistoryDisplay extends Component {
  ...
  async update() {
    ...
    let content = await fetch(
      `https://ossci-job-status.s3.amazonaws.com/single/${branch.replace(
        "/",
        "_"
      )}.json.gz`
    );
    let allStatuses = await content.json();
    const currentTime = new Date();

    const builds = [];
    for (const build of allStatuses) {
      let message = build.head_commit_title;
      if (build.pr) {
        message += ` (#${build.pr})`;
      }
      let user = build.head_commit_author_username;
      if (!user) {
        user = "<unknown>";
      }
      let sb_map = new Map();
      for (const [jobName, job] of Object.entries(build.jobs)) {
        if (job !== null) {
          sb_map.set(jobName, {
            build_url: job[1],
            status: job[0],
      ...
      builds.push({
        ...
        id: build.head_commit_id,
        timestamp: build.head_commit_timestamp,
        message: message,
        sb_map: sb_map,
      });
    }
